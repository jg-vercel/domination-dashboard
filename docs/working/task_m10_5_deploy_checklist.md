...
| --- | --- | --- | --- |
| `GOOGLE_CLIENT_ID` | 설정 완료 | 미설정 | 제한 정보 |
| `GOOGLE_CLIENT_SECRET` | 설정 완료 | 미설정 | 예 |
...
| Google 로그인 시작 | `/api/auth/google/start`가 `307`로 Google OAuth authorization endpoint 이동 |
| 비로그인 session keep-alive | `POST /api/auth/session`이 `401 AUTH_REQUIRED`, `no-store` |
| 실계정·실수령 | 미실행 |

Google OAuth 변수 반영 재배포는 2026-08-31 완료됐고 고정 별칭 `https://domination-dashboard-preview.vercel.app`이 새 Preview를 가리킵니다. 작업지시자의 브라우저 로그인 후 refresh token 필수 수신, opaque cookie, 180일 Redis rolling TTL, dashboard keep-alive, logout Redis 삭제를 추가로 확인합니다.
